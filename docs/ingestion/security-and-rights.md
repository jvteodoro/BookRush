...
audience e JWKS configuráveis pelo starter compartilhado. O validador verifica assinatura, issuer,
...
O issuer configurado hoje é o realm de produto durante a compatibilidade
existente. O starter nunca aceita um issuer escolhido pelo token; a migração de
rotas administrativas para `bookrush-platform` será delimitada por endpoint.
