...
O provider Docker observa somente serviços com `traefik.enable=true` na rede `bookrush`:
...
Ao adicionar um microserviço, publique apenas a porta interna na rede Docker e acrescente labels Traefik no `compose.yaml`. Não publique a porta do novo serviço no host. O socket Docker é montado somente como leitura para descoberta; o dashboard do Traefik permanece desativado.
